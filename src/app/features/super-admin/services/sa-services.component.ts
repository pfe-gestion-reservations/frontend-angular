...
@Component({
  ...
  templateUrl: './sa-services.component.html',
  styleUrls: ['./sa-services.component.css']
})
export class SaServicesComponent implements OnInit {
  ...
  get filteredServices(): ServiceResponse[] {
    ...
    return this.services.filter(s => {
      const matchEnt = !this.selectedEntrepriseId || s.entrepriseId === this.selectedEntrepriseId;
      const matchQ   = !q || s.nom.toLowerCase().includes(q) ||
                       (s.description ?? '').toLowerCase().includes(q) ||
                       (this.getEntNom(s.entrepriseId) ?? '').toLowerCase().includes(q);
  ...
  selectEnt(e: EntrepriseResponse): void { this.selectedEntrepriseId = e.id; this.entDropOpen = false; }

  clearEnt(): void {
    this.selectedEntrepriseId = null; this.entSearch = '';
    this.filteredEnts = [...this.entreprises]; this.entDropOpen = false;
  }
  ...
  openEdit(s: ServiceResponse): void {
    this.editingService = s; this.selectedEntrepriseId = s.entrepriseId;
    const c = this.configs.get(s.id);
  ...
  save(): void {
    ...
    if (this.form.invalid || !this.selectedType || !this.selectedEntrepriseId) return;
 
    const v = this.form.getRawValue();
 
    // ── Validation capacité min/max ──
    if (this.selectedType === 'RESSOURCE_PARTAGEE') {
      ...
      if (cMin > cMax) {
        this._showValidationError({
          ...
          message: 'Le nombre de personnes minimum ne peut pas être supérieur au maximum.',
          hint: `Valeurs saisies : min = <strong>${cMin}</strong>, max = <strong>${cMax}</strong>`
        });
        return;
      }
      if (!this.editingService && this.inlineRessources.length === 0) {
        this._showValidationError({
          ...
          message: 'Au moins une ressource est obligatoire pour un service de type <strong>Ressource partagée</strong>.',
          hint: 'Cliquez sur « Ajouter une ressource » pour en créer une avant de continuer.'
        });
        return;
      }
    }
    ...
    const serviceBody: any = {
      ...
      entrepriseId: this.selectedEntrepriseId, typeService: this.selectedType
    };
    ...
    if (!this.editingService && this.selectedType !== 'RESSOURCE_PARTAGEE') {
      const doublon = this.services.filter(s => s.entrepriseId === this.selectedEntrepriseId)
        .find(s => s.nom.trim().toLowerCase() === (v.nom || '').trim().toLowerCase() &&
      ...
      if (doublon) {
        ...
        this._showValidationError({
          ...
          message: `Un service identique existe déjà pour cette entreprise.`,
          hint: `Nom : <strong>${doublon.nom}</strong> — ${doublon.dureeMinutes} min — ${doublon.tarif != null ? doublon.tarif + ' DT' : 'Gratuit'}`
        });
        return;
      }
    }
        ...
        error: (err) => {
          ...
          if (err?.status === 409) {
            this._showValidationError({
              ...
              iconColor: '#f59e0b',
              title: 'Service déjà existant',
              message: err?.error?.message || 'Un service identique existe déjà.',
              hint: null
            });
          } else {
  ...
  private _showDeleteConfirm(s: ServiceResponse): void {
    ...
    this.renderer.setStyle(overlay, 'background', 'rgba(0,0,0,0.6)'); this.renderer.setStyle(overlay, 'z-index', '99999');
    this.renderer.setStyle(overlay, 'display', 'flex'); this.renderer.setStyle(overlay, 'align-items', 'center');
    this.renderer.setStyle(overlay, 'justify-content', 'center'); this.renderer.setStyle(overlay, 'backdrop-filter', 'blur(4px)');
    const box = this.renderer.createElement('div');
    this.renderer.setStyle(box, 'background', bg); this.renderer.setStyle(box, 'border', `1px solid ${isDark ? 'rgba(239,68,68,.25)' : '#fecaca'}`);
    this.renderer.setStyle(box, 'border-radius', '20px'); this.renderer.setStyle(box, 'padding', '32px 28px');
    this.renderer.setStyle(box, 'text-align', 'center'); this.renderer.setStyle(box, 'max-width', '400px'); this.renderer.setStyle(box, 'width', '92%');
    ...
    const rpNote = isRP ? `<div style="display:flex;align-items:center;gap:10px;background:rgba(16,185,129,.08);
      border:1px solid rgba(16,185,129,.2);border-radius:10px;padding:10px 14px;margin-bottom:16px;text-align:left;font-size:.8rem;color:#34d399">
      <i class="fas fa-layer-group" style="flex-shrink:0"></i>
      <span>Les ressources associées seront <strong>supprimées automatiquement</strong>.</span></div>` : '';

    box.innerHTML = `
      <div style="width:52px;height:52px;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.3);
           border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;margin:0 auto 16px;color:#ef4444">
        <i class="fas fa-trash-alt"></i></div>
      <div style="font-size:1rem;font-weight:700;color:${text};margin-bottom:6px">Supprimer ce service ?</div>
      <div style="font-size:.82rem;color:${muted};margin-bottom:14px"><strong style="color:${text}">${s.nom}</strong></div>
      ${rpNote}
      <div style="font-size:.75rem;color:#ef4444;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);
        ...
        <i class="fas fa-exclamation-triangle" style="margin-right:5px"></i>Cette action est irréversible.</div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button id="del-cancel" style="background:${cancelBg};color:${cancelCl};border:1px solid ${cancelBd};
          padding:9px 20px;border-radius:8px;font-size:.82rem;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
        <button id="del-ok" style="background:#ef4444;color:#fff;border:none;padding:9px 22px;border-radius:8px;
          font-size:.82rem;font-weight:700;cursor:pointer;font-family:inherit">
          <i class="fas fa-trash-alt" style="margin-right:5px"></i>Supprimer</button>
      </div>`;
  ...
  private _openLinkedDialog(s: ServiceResponse, nbRes: number, nbFile: number, nbRessources: number): void {
    ...
    this.renderer.setStyle(overlay, 'justify-content', 'center'); this.renderer.setStyle(overlay, 'backdrop-filter', 'blur(4px)');
    const box = this.renderer.createElement('div');
    this.renderer.setStyle(box, 'background', bg); this.renderer.setStyle(box, 'border', `1px solid ${isDark ? 'rgba(255,255,255,.1)' : '#e2e2f0'}`);
    this.renderer.setStyle(box, 'border-radius', '20px'); this.renderer.setStyle(box, 'padding', '28px 24px');
    this.renderer.setStyle(box, 'text-align', 'center'); this.renderer.setStyle(box, 'max-width', '450px'); this.renderer.setStyle(box, 'width', '92%');
    this.renderer.setStyle(box, 'box-shadow', isDark ? '0 24px 64px rgba(0,0,0,.6)' : '0 16px 48px rgba(0,0,0,.15)');
    this.renderer.setStyle(box, 'font-family', 'inherit');
    const close = () => this.renderer.removeChild(document.body, overlay);
    const items: string[] = [];

    if (nbRes > 0) items.push(`<div style="display:flex;align-items:center;gap:12px;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.18);border-radius:12px;padding:12px 14px;text-align:left">
      <div style="width:34px;height:34px;background:rgba(239,68,68,.1);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#ef4444"><i class="fas fa-calendar-alt"></i></div>
      <div><div style="color:#ef4444;font-weight:700;font-size:.82rem;margin-bottom:2px">${nbRes} réservation${nbRes>1?'s':''}</div>
      <div style="font-size:.74rem;color:${sub}">Réservations actives liées à ce service</div></div></div>`);

    if (nbFile > 0) items.push(`<div style="display:flex;align-items:center;gap:12px;background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.18);border-radius:12px;padding:12px 14px;text-align:left">
      <div style="width:34px;height:34px;background:rgba(245,158,11,.1);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#f59e0b"><i class="fas fa-list-ol"></i></div>
      <div><div style="color:#f59e0b;font-weight:700;font-size:.82rem;margin-bottom:2px">${nbFile} entrée${nbFile>1?'s':''} en file d'attente</div>
      <div style="font-size:.74rem;color:${sub}">File d'attente active pour ce service</div></div></div>`);

    if (nbRessources > 0) items.push(`<div style="display:flex;align-items:center;gap:12px;background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.18);border-radius:12px;padding:12px 14px;text-align:left">
      <div style="width:34px;height:34px;background:rgba(99,102,241,.1);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#818cf8"><i class="fas fa-layer-group"></i></div>
      <div><div style="color:#818cf8;font-weight:700;font-size:.82rem;margin-bottom:2px">${nbRessources} ressource${nbRessources>1?'s':''}</div>
      <div style="font-size:.74rem;color:${sub}">Ressources encore associées</div></div></div>`);

    const itemsHtml = items.length > 0
      ? `<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;text-align:left">${items.join('')}</div>`
      : `<div style="font-size:.88rem;color:${muted};margin-bottom:16px">Ce service est lié à des <strong style="color:${text}">réservations ou configurations</strong> existantes.</div>`;

    box.innerHTML = `
      <div style="width:48px;height:48px;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.3);
           border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.2rem;margin:0 auto 14px;color:#f59e0b">
        <i class="fas fa-exclamation-triangle"></i></div>
      <div style="font-size:.98rem;font-weight:700;color:${text};margin-bottom:6px">Suppression impossible</div>
      <div style="font-size:.8rem;color:${muted};margin-bottom:16px">
        <strong style="color:${text}">${s.nom}</strong> est encore lié aux éléments suivants :</div>
      ${itemsHtml}
      <div style="font-size:.74rem;color:${sub};background:${hintBg};border-radius:10px;padding:10px 12px;
           border:1px solid ${hintBd};margin-bottom:18px;text-align:left">
        <i class="fas fa-lightbulb" style="color:#6366f1;margin-right:5px"></i>
        Supprimez ou dissociez ces éléments, puis réessayez.</div>
      <button id="linked-ok" style="background:linear-gradient(135deg,#6366f1,#4f46e5);color:#fff;border:none;
        padding:10px 0;border-radius:10px;font-size:.85rem;font-weight:700;cursor:pointer;width:100%;font-family:inherit">
        <i class="fas fa-check" style="margin-right:6px"></i>Compris</button>`;
    this.renderer.appendChild(overlay, box); this.renderer.appendChild(document.body, overlay);
  ...
  private _confirmAction(opts: {
    ...
    message: string; confirmLabel: string; confirmColor: string; onConfirm: () => void;
  }): void {
    const isDark = document.documentElement.getAttribute('data-theme') !== 'light';
    const bg    = isDark ? '#16161f' : '#ffffff';
    const text  = isDark ? '#f2f2f8' : '#0f0f1a';
    ...
    const close = () => this.renderer.removeChild(document.body, overlay);
    box.innerHTML = `
      <div style="width:48px;height:48px;background:${opts.iconColor}18;border:1px solid ${opts.iconColor}40;
           border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;margin:0 auto 14px;color:${opts.iconColor}">
        <i class="${opts.icon}"></i></div>
      <div style="font-size:1rem;font-weight:700;color:${text};margin-bottom:${opts.subtitle?'4px':'8px'}">${opts.title}</div>
      ${opts.subtitle ? `<div style="font-size:.85rem;color:#60a5fa;font-weight:600;margin-bottom:8px">${opts.subtitle}</div>` : ''}
      <div style="font-size:.875rem;color:${muted};margin-bottom:22px;line-height:1.6">${opts.message}</div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button id="ca-cancel" style="background:${cancelBg};color:${cancelCl};border:1px solid ${cancelBd};padding:9px 20px;border-radius:8px;font-size:.82rem;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
        <button id="ca-ok" style="background:${opts.confirmColor};color:#fff;border:none;padding:9px 22px;border-radius:8px;font-size:.82rem;font-weight:700;cursor:pointer;font-family:inherit">
          <i class="${opts.icon}" style="margin-right:5px"></i>${opts.confirmLabel}</button>
      </div>`;
    this.renderer.appendChild(overlay, box); this.renderer.appendChild(document.body, overlay);
  ...
  private _showValidationError(opts: {
    icon: string;
    iconColor: string;
    title: string;
    message: string;
    ...
    const hintHtml = opts.hint
      ? `<div style="font-size:.74rem;color:${muted};background:${hintBg};border-radius:10px;padding:10px 12px;
           border:1px solid ${hintBd};margin-bottom:18px;text-align:left;line-height:1.6">
           <i class="fas fa-info-circle" style="color:#6366f1;margin-right:5px"></i>${opts.hint}
         </div>`
      : '';
 
    box.innerHTML = `
      <div style="width:52px;height:52px;background:${opts.iconColor}18;border:1px solid ${opts.iconColor}40;
           border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;margin:0 auto 16px;color:${opts.iconColor}">
        <i class="${opts.icon}"></i></div>
      <div style="font-size:1rem;font-weight:700;color:${text};margin-bottom:8px;letter-spacing:-0.01em">${opts.title}</div>
      <div style="font-size:.82rem;color:${muted};margin-bottom:${opts.hint ? '14px' : '22px'};line-height:1.6">${opts.message}</div>
      ${hintHtml}
      <button id="val-ok" style="background:${btnBg};color:#fff;border:none;padding:10px 0;border-radius:10px;
        font-size:.85rem;font-weight:700;cursor:pointer;width:100%;font-family:inherit;
