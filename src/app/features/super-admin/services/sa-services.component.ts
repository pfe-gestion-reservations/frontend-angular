  ...
  <div>
    ...
    <div class="dup-overlay" *ngIf="showDuplicatePopup" (click)="showDuplicatePopup=false">
      <div class="dup-popup" (click)="$event.stopPropagation()">
        ...
        <div class="dup-msg">Un service avec le même nom, prix et durée existe déjà dans cette entreprise.</div>
...
export class SaServicesComponent implements OnInit {
  ...
  get filteredServices(): ServiceResponse[] {
    const list = this.services.filter(s => {
      ...
      const matchArchived = this.showArchived ? true : !s.archived;
      return matchEnt && matchArchived;
    });
    return [...list.filter(s => !s.archived), ...list.filter(s => s.archived)];
  }
  ...
  save(): void {
    ...
    const serviceBody = { nom: v.nom!, description: v.description || '', dureeMinutes: v.dureeMinutes ?? 0, tarif: v.tarif ?? null, entrepriseId: this.selectedEntrepriseId, typeService: this.selectedType };
    ...
    if (this.editingService) {
      ...
      this.api.createService(serviceBody).subscribe({
        next: (s) => {
          this.api.saveConfigService({ ...configBody, serviceId: s.id } as any).subscribe({
            next: () => {
              if (this.selectedType === 'RESSOURCE_PARTAGEE' && this.inlineRessources.length > 0) {
                const ressourceCreations = this.inlineRessources.map(r =>
                  this.api.createRessource({ ...r, serviceId: s.id })
                );
                let done = 0;
                ressourceCreations.forEach(obs => obs.subscribe({
                  next: () => { done++; if (done === ressourceCreations.length) { this.toast.success('Service créé avec ' + done + ' ressource(s) !'); this.reloadAll(); this.closeModal(); this.loading = false; } },
                  error: () => { done++; if (done === ressourceCreations.length) { this.toast.success('Service créé (certaines ressources ont échoué)'); this.reloadAll(); this.closeModal(); this.loading = false; } }
                }));
              } else {
                this.toast.success('Service créé !'); this.reloadAll(); this.closeModal(); this.loading = false;
              }
            },
            error: () => { this.toast.error('Config échouée'); this.loading = false; }
          });
  ...
  archiver(s: ServiceResponse): void {
    this.api.archiverService(s.id).subscribe({ next: () => { this.toast.success('Archivé'); this.api.getServices().subscribe(d => this.services = d); }, error: () => this.toast.error('Erreur') });
  }
  desarchiver(s: ServiceResponse): void {
    this.api.desarchiverService(s.id).subscribe({ next: () => { this.toast.success('Désarchivé'); this.api.getServices().subscribe(d => this.services = d); }, error: () => this.toast.error('Erreur') });
  }
